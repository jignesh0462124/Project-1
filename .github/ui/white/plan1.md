...
Two themes, one token system. Same variable names resolve to different values per theme — components never branch on theme, they just read tokens. Dark is two signal colors on near-black; light is the same two signal colors deepened slightly for contrast, on warm-white. No more than two saturated hues on screen at once, in either theme.
...
| `--on-accent` | `#0B0D12` | Text/icons placed on top of `--accent` or `--accent-2` fills |

**Light theme**

| Token | Hex | Role |
|---|---|---|
| `--ink` | `#F6F5F1` | Page background — warm off-white, not stark `#FFFFFF` (matches the editor's need to sit easy on the eyes for long sessions) |
| `--surface` | `#FFFFFF` | Panel / rail background |
| `--surface-raised` | `#FBFAF7` | Cards, inputs, editor tabs |
| `--surface-hover` | `#F0EFEA` | Hover state on raised surfaces |
| `--line` | `#E2E0D8` | Default hairline border |
| `--line-strong` | `#C9C6BA` | Emphasized border (active tab, focused input) |
| `--text` | `#15171C` | Primary text |
| `--text-dim` | `#535863` | Secondary text |
| `--muted` | `#8A8F99` | Tertiary text, placeholders, line numbers |
| `--accent` | `#0E9E6E` | Mint, deepened for AA contrast on white — "you," live/connected, primary actions |
| `--accent-dim` | `#DEF5EA` | Mint at low opacity for backgrounds/badges |
| `--accent-2` | `#7A4FE0` | Violet, deepened for AA contrast on white — owner, AI, secondary actions |
| `--accent-2-dim` | `#EBE3FB` | Violet at low opacity for backgrounds/badges |
| `--warn` | `#B5740C` | Caution, pending states (deepened amber — raw `#FFB454` fails AA on white) |
| `--danger` | `#D6433C` | Destructive, errors, disconnected (deepened red) |
| `--success` | `#0E9E6E` | Reuses accent, same logic as dark |
| `--on-accent` | `#FFFFFF` | Text/icons placed on top of `--accent` or `--accent-2` fills |

> Rule: badge/status color vocabulary is capped at five meanings (you, owner, warn, danger, neutral) in both themes. Anything beyond that uses `--text-dim` and a text label instead of inventing a sixth color.

> Why hues shift between themes rather than reusing the exact dark hex on white: `#6FF0BD` mint and `#B79CFF` violet are tuned for contrast against near-black and fail WCAG AA as text/icon color on a light background. The same logic applies to `--warn` and `--danger`. Surfaces, lines, and fills can reuse a hue's *family* across themes; small text and icons cannot reuse the literal hex once the background flips.

Player cursor colors (assigned round-robin per session, never reused within a room) — same logic, theme-aware:

| Slot | Name | Dark hex | Light hex |
|---|---|---|---|
| 1 | Mint | `#6FF0BD` | `#0E9E6E` |
| 2 | Violet | `#B79CFF` | `#7A4FE0` |
| 3 | Amber | `#FFB454` | `#B5740C` |
| 4 | Sky | `#7BB8FF` | `#2E7DD1` |

Cursor color identity (which slot a player is) stays stable across a theme switch — only the rendered hex changes, so a mid-session light/dark toggle never reads as a different person.
...
Neo-brutalist hard-offset shadow, used sparingly — only on the *one* currently-active/focused surface, not on every card. The offset color is theme-aware (it's just `--accent` / `--accent-2`, which already shift per theme per §3.1); only the floating-element shadow needs a distinct dark/light value, since a soft black shadow that reads correctly on near-black is too heavy on white.

| Token | Value (dark) | Value (light) | Usage |
|---|---|---|---|
| `--shadow-flat` | none | none | Default panel — flat, just a hairline border |
| `--shadow-active` | `2px 2px 0 0 var(--accent)` | `2px 2px 0 0 var(--accent)` | The focused editor tab, the primary CTA, your own player row |
| `--shadow-active-violet` | `2px 2px 0 0 var(--accent-2)` | `2px 2px 0 0 var(--accent-2)` | Owner-only controls, AI panel header |
| `--shadow-pop` | `0 8px 24px -4px rgba(0,0,0,0.5)` | `0 8px 24px -4px rgba(21,23,28,0.14)` | Floating elements: toasts, dropdowns, modals |
...
- Color contrast: body text `--text` on `--ink`/`--surface` exceeds WCAG AA (4.5:1) at all defined sizes, verified independently for dark and light — the two palettes are not the same hues with backgrounds swapped, so contrast was checked per-theme rather than assumed to carry over; `--muted` is reserved for non-essential metadata only, never for content the user must read, in either theme
...
- All color tokens above should ship as CSS custom properties, scoped via a `data-theme="dark"` / `data-theme="light"` attribute on `<html>` (or Tailwind's `dark:` variant mapped to the same token names) — both palettes in §3.1 are first-class and complete, not a follow-up pass
- Persist the user's theme choice (localStorage or account preference) and default to system `prefers-color-scheme` on first visit, with the manual toggle in the top bar (landing) / header (room workspace) able to override it
